...
namespace Sumorin.GameFramework.GameManager
{
	...
	public class SimpleLocalizedStringDrawer: OdinValueDrawer<LocalizedString>
	{
		...
		/// <summary>
		/// 取得目前 entry 在主要語言（Project Locale）下的翻譯值，無則回空字串
		/// </summary>
		/// <returns>主要語言翻譯值</returns>
		private string GetPrimaryLocaleValue()
		{
			if(selectedCollection == null || selectedEntry == null) return "";
			...
			return table?.GetEntry(selectedEntry.Id)?.Value ?? "";
		}
		...
		private void ShowPicker(SerializedProperty property)
		{
			...
			selector.SelectionConfirmed += selection =>
			{
				...
				if(selected.Collection == null || selected.SharedEntry == null) return;
		...
		private void DrawTableDetails(SerializedProperty property)
		{
			...
			{
				var openButtonRect = new Rect(tableSelectionRect.xMax, tableRect.y, OpenTableEditorButtonWidth, tableRect.height);

				if(GUI.Button(openButtonRect, tableWindowIcon))
				{
					// 使用反射開啟 LocalizationTablesWindow
					var windowType = Type.GetType("UnityEditor.Localization.UI.LocalizationTablesWindow, Unity.Localization.Editor");
			...
			if(selectedTableIndex == 0)
			{
				...
				{
					var windowType = Type.GetType("UnityEditor.Localization.UI.TableCreatorWindow, Unity.Localization.Editor");
		...
		private void DrawLocalePreview()
		{
			...
			if(projectLocales == null || projectLocales.Count == 0) return;
			...
			foreach(var locale in projectLocales)
			{
				...
				var borderColor = Event.current.IsMouseOver(fieldRect) ? OdinLocalizationGUI.RowBorderHover : OdinLocalizationGUI.RowBorder;
		...
		private void DrawEntryName(SerializedProperty property)
		{
			...
			if(EditorGUI.EndChangeCheck() && selectedCollection != null && selectedEntry != null)
			{
				...
				{
					entryNameLabel = new GUIContent(
						Styles.EntryName.text,
	...
	public class LocalizedStringEntrySelector: OdinSelector<LocalizedStringEntrySelector.EntrySelection>
	{
		...
		private void RebuildTreeWithFilter()
		{
			...
			foreach(var kvp in collectionEntries)
			{
				...
				// 找出匹配的 entries
				var matchingEntries =
					hasSearch ? entries.Where(e => e.SearchString.Contains(searchTerm, StringComparison.OrdinalIgnoreCase)).ToList() : entries;
		...
		protected override void BuildSelectionTree(OdinMenuTree tree)
		{
			...
			{
				if(collection == null || collection.SharedData == null) continue;

				if(!collection.SharedData.Entries.Any()) continue;
		...
		private void DrawEntryValues(OdinMenuItem item, StringTableCollection collection, SharedTableData.SharedTableEntry entry)
		{
			...
			if(valuesToShow.Count == 0) return;
