...
namespace Sumorin.GameManagerBase
{
	...
	public abstract class SODataBase: SerializedScriptableObject, IConfig
	{
		...
		/// </summary>
		/// <remarks>
		/// 值與資產檔名永遠一致，改名由欄位下方的確認按鈕發起，見 <see cref="AssetRenamer.TryRename" />。
		...
		private AssetNameRenameField RenameField => renameField ??= new(this);

		private Color AssetNameFieldColor => RenameField.FieldColor;

		/// <remarks>
		/// 資產從磁碟反序列化回來時不會帶著這個物件，取用時才建。
		/// </remarks>
		private AssetNameRenameField renameField;
		...
		/// <remarks>
		/// <see cref="AssetNameRenameField" /> 用它決定確認按鈕能不能按，所以不是 private。
		/// </remarks>
		internal bool IsAssetNameLegal()
		{
		...
		private void ClearRenameError() => RenameField.ClearError();

		private void TrackAssetNameEdit() => RenameField.TrackEdit();

		private void DrawRenameUi() => RenameField.Draw();
