...
namespace Sumorin.Localization.Editor
{
	...
	public class SumorinLocalizationEditorWindow: OdinLocalizationEditorWindow
	{
		...
		protected override OdinMenuTree BuildMenuTree()
		{
			...
			tree.Add("設定", new SumorinLocalizationConfig(), SdfIconType.GearFill);
					...
					case SelectionChangedType.ItemAdded:
						...
						switch(tree.Selection.SelectedValue)
						{
							case OdinAssetTableCollectionEditor assetCollection:
								...
								if(assetCollection.SelectionType == OdinTableSelectionType.TableEntry && State.CurrentTopTab == RightMenuTopTabs.Metadata)
								{
							...
							case OdinStringTableCollectionEditor stringCollection:
								...
								if(stringCollection.SelectionType == OdinTableSelectionType.TableEntry && State.CurrentTopTab == RightMenuTopTabs.Metadata)
								{
		...
		protected override void OnImGUI()
		{
			...
			if(Event.current.keyCode == KeyCode.Tab && (Event.current.type == EventType.KeyDown || Event.current.rawType == EventType.KeyDown))
			{
		...
		private void DrawExtendedToolbarBackground()
		{
			...
			EditorGUI.DrawRect(toolbarBgRect, EditorGUIUtility.isProSkin ? new Color(0.22f, 0.22f, 0.22f) : new Color(0.76f, 0.76f, 0.76f));

			// 繪製底部分隔線
			EditorGUI.DrawRect(
				new Rect(0, ExtendedToolbarHeight - 1, position.width, 1),
				EditorGUIUtility.isProSkin ? new Color(0.15f, 0.15f, 0.15f) : new Color(0.6f, 0.6f, 0.6f)
			);
		}
		...
		private void HandleKeyboardShortcuts()
		{
			...
			if(!(MenuTree?.Selection?.SelectedValue is SumorinStringTableCollectionEditor ctrlEditor)) return;
		...
		private void DrawExtendedToolbarButtons()
		{
			...
			{
				var path = EditorUtility.SaveFilePanel("Export CSV", "", editor.Collection.name, "csv");
				...
				if(!string.IsNullOrEmpty(path))
				{
					try
					{
						...
						ShowToast(ToastPosition.BottomLeft, SdfIconType.Check2, $"Exported to {path}", new Color(0.29f, 0.57f, 0.42f), 8.0f);
					}
			...
			if(GUI.Button(importRect, "Import CSV", SirenixGUIStyles.MiniButton))
			{
				...
				if(!string.IsNullOrEmpty(path))
				{
					try
					{
						...
						ShowToast(ToastPosition.BottomLeft, SdfIconType.Check2, $"Imported {count} entries", new Color(0.29f, 0.57f, 0.42f), 8.0f);
		...
		private void HandleDragHandleClick(OdinStringTableCollectionEditor editor)
		{
			...
			// 檢查滑鼠是否在拖曳區域內
			if(mousePos.x >= dragAreaX && mousePos.x <= dragAreaRight && mousePos.y >= dragAreaTop && mousePos.y <= dragAreaBottom)
			{
				// 計算點擊的是哪一行（考慮捲動位置）
		...
		{
			if(MenuTree?.Selection?.SelectedValue is OdinStringTableCollectionEditor editor) return editor;
		...
		private Vector2 CalculateTargetCellPosition(SumorinStringTableCollectionEditor editor)
		{
			...
			// 計算 Y 位置（考慮滾動）
			var scrollY = editor.EntryScrollView.PositionY;
			var cellY = ExtendedToolbarHeight + originalToolbarHeight + columnHeight + targetRow * rowHeight - scrollY + rowHeight / 2;
